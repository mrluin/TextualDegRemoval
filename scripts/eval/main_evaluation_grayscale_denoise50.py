...
if __name__ == '__main__':
    ...
    for gt_path, ref_path in zip(gt_img_paths, ref_img_paths):
        f = open(gt_path, "rb")
        img_bytes = f.read()
        gt_img = imfrombytes(img_bytes, flag="grayscale", float32=True)
        gt_img = np.expand_dims(gt_img, axis=2)

        f = open(ref_path, "rb")
        img_bytes = f.read()
        ref_img = imfrombytes(img_bytes, flag="grayscale", float32=True)
        ref_img = np.expand_dims(ref_img, axis=2)

        lq_img = gt_img.copy()

        np.random.seed(seed=0)
        lq_img += np.random.normal(0, 50 / 255.0, lq_img.shape)
