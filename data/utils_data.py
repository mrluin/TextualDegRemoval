...
from utils.utils_image import img2tensor
from utils.utils_misc import scandir
...
def tri_paths_from_folder(folders, keys, filename_tmpl):
    ...
    assert len(folders) == 3, (
        'The len of folders should be 3 with [input_folder, gt_folder, ref_folder]. '
        f'But got {len(folders)}')
    assert len(keys) == 3, (
        'The len of keys should be 2 with [input_key, gt_key, ref_key]. '
        f'But got {len(keys)}')
    input_folder, gt_folder, ref_folder = folders
    input_key, gt_key, ref_key = keys
    ...
    for idx in range(len(gt_paths)):
        ...
        basename, ext = osp.splitext(osp.basename(gt_path))
        input_path = input_paths[idx]
        ref_path = ref_paths[idx]
        basename_input, ext_input = osp.splitext(osp.basename(input_path))
        basename_ref, ext_ref = osp.splitext(osp.basename(ref_path))

        input_name = f'{filename_tmpl.format(basename)}{ext_input}'
        input_path = osp.join(input_folder, input_name)

        ref_name = f'{filename_tmpl.format(basename)}{ext_ref}'
        ref_path = osp.join(ref_folder, ref_name)

        assert input_name in input_paths, (f'{input_name} is not in '
                                           f'{input_key}_paths.')

        assert ref_name in ref_paths, (f'{ref_name} is not in '
                                       f'{ref_key}_paths.')
...
def tri_DP_paths_from_folder(folders, keys, filename_tmpl):

    assert len(folders) == 4, (
        'The len of folders should be 4 with [inputL_folder, inputR_folder, gt_folder, ref_folder]. '
        f'But got {len(folders)}')
    assert len(keys) == 4, (
        'The len of keys should be 4 with [inputL_key, inputR_key, gt_key, ref_key]. '
        f'But got {len(keys)}')

    inputL_folder, inputR_folder, gt_folder, ref_folder = folders
    inputL_key, inputR_key, gt_key, ref_key = keys
    ...
    for idx in range(len(gt_paths)):
        ...
        ref_path = ref_paths[idx]
        basename_input, ext_input = osp.splitext(osp.basename(ref_path))
        ref_name = f'{filename_tmpl.format(basename)}{ext_input}'
        ref_path = osp.join(ref_folder, ref_name)
        assert ref_name in ref_paths, (f'{ref_name} is not in ' 
                                       f'{ref_key}_paths.')
